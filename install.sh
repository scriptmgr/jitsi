...
__write_nginx_vhost() {
  # Skip if nginx vhost writing is disabled or nginx is not installed
  ...
  __info "Writing nginx vhost: ${NGINX_VHOST_FILE}"
  cat > "${NGINX_VHOST_FILE}" <<NGINX
# Jitsi Meet — generated by install.sh on ${_generated_at}
# Public URL: ${PUBLIC_URL}
# To regenerate: PUBLIC_URL=${PUBLIC_URL} sudo -E bash install.sh
...
# room.${PUBLIC_DOMAIN}/ → same-domain 302 → room.${PUBLIC_DOMAIN}/room → proxy to prosody
# Requires a *.${PUBLIC_DOMAIN} wildcard SSL cert.
server {
    listen 443 ssl;
...
NGINX
  ...
  # Check whether any nginx config already includes vhosts.d; if not, create the include file
  if ! grep -rq -- 'vhosts\.d' /etc/nginx/ 2>/dev/null; then
    local _include_file="/etc/nginx/conf.d/jitsi-vhosts.conf"
    __info "No vhosts.d include found — creating ${_include_file}"
  ...
  if _test_out="$(nginx -t 2>&1)"; then
    ...
    __info "nginx reloaded successfully."
  else
    __warn "nginx config test failed — check the vhost file before reloading."
    __warn "${_test_out}"
  fi
}
